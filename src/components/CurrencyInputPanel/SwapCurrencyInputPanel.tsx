...
const CurrencySelect = styled(ButtonGray)<{
  ...
  align-items: center;
  background-color: ${({ selected }) => (selected ? '#FFF' : '#F90000')};
  opacity: ${({ disabled }) => (!disabled ? 1 : 0.4)};
  color: ${({ selected, theme }) => (selected ? '#222' : theme.white)};
  cursor: pointer;
  height: 36px;
  border-radius: 18px;
  outline: none;
  user-select: none;
  border: 1px solid ${({ selected, theme }) => (selected ? theme.surface3 : '#0038FF')};
  font-size: 19px;
  font-weight: 600;
  width: ${({ hideInput }) => (hideInput ? '100%' : 'initial')};
  ...
  &:hover,
  &:active {
    background-color: ${({ theme, selected }) => (selected ? '#EEE' : '#F90000DD')};
  }
