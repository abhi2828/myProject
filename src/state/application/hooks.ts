...
export function useSwapModalIsOpen(): boolean {
  const openSwapModal = useAppSelector((state: AppState) => state.application.openSwapModal)
...
export function useToggleSwapModal(): () => void {
  const isOpen = useSwapModalIsOpen()
  const dispatch = useAppDispatch()
  return useCallback(() => dispatch(setSwapModal(!isOpen)), [dispatch, isOpen])
}
